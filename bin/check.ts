...
const scenarioRoster = [...foundationRoster, ...jointsRoster, ...surfacesRoster];
if (scenarioCorpus.schema !== "box3d-oracle/scenario-command/v1" || JSON.stringify(scenarioCorpus.scenarios?.map((scenario) => scenario.name)) !== JSON.stringify(scenarioRoster) || scenarioCorpus.scenarios?.some((scenario) => scenario.id !== `s1.${scenario.name}.v1`)) throw new Error("scenario corpus membership is not the exact cumulative O5c roster/ID join");
...
if (!scenarioAdapter.includes("SCENARIO_MESH_RESOURCE") || !scenarioAdapter.includes("SCENARIO_HEIGHT_RESOURCE") || !scenarioAdapter.includes("isBullet")) throw new Error("surface geometry and continuous body commands are not interpreted");
