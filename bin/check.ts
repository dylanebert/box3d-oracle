...
const required = ["README.md", "package.json", "bin/oracle.ts", "bin/check.ts", "test/oracle.test.ts", "scenarios/v1.json", "scenarios/v2.json", "adapter/scenario.c", "adapter/scenario_main.c", "adapter/scenario_commands.h", "include/box3d_oracle_adapter.h", "adapter/main.c", "adapter/math.c", "adapter/geometry.c", "adapter/distance.c", "adapter/tree.c", "adapter/manifold.c", "adapter/query.c", "adapter/mover.c", "adapter/writer.c", "adapter/o3_main.c", "adapter/o4_main.c", "adapter/o4.c", "adapter/whitebox.c", "hooks/patches.ts", "hooks/oracle_hooks.h", "schema/v1.json"];
...
if (!oracle.includes("generateBundleV4") || !oracle.includes("corpusVersion") || !oracle.includes("SCENARIO_NAMES") || !oracle.includes("readsShallot: false") || !oracle.includes("readsGolds: false")) throw new Error("O5 scenario corpus and anti-gold lanes are incomplete");
