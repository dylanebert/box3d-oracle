...
#define SCENARIO_BODY_CAPACITY 512
#define SCENARIO_RESOURCE_CAPACITY 1024
#define SCENARIO_SHAPE_CAPACITY 1024
...
{
    if (c->a < 0 || c->a >= SCENARIO_BODY_CAPACITY || c->b < 0 || c->b >= SCENARIO_BODY_CAPACITY || !b3Body_IsValid(bodies[c->a]) || !b3Body_IsValid(bodies[c->b])) return 0;
    switch (c->op) {
...
static void shape_label(FILE* out, b3ShapeId id, b3ShapeId* shapes) { for (int i = 0; i < SCENARIO_SHAPE_CAPACITY; ++i) if (same_shape(id, shapes[i])) { fprintf(out, "s%d", i); return; } fputs("unknown", out); }

static int run_scenario(int index, FILE* out)
{
    if (index < 0 || index >= scenario_record_count) return 2;
    const ScenarioRecord* record = scenario_records + index; b3WorldId world = b3_nullWorldId; b3BodyId bodies[SCENARIO_BODY_CAPACITY] = { 0 }; b3ShapeId shapes[SCENARIO_SHAPE_CAPACITY] = { 0 }; b3BoxHull boxes[SCENARIO_RESOURCE_CAPACITY]; b3Sphere spheres[SCENARIO_RESOURCE_CAPACITY]; b3Capsule capsules[SCENARIO_RESOURCE_CAPACITY]; b3HullData* hulls[SCENARIO_RESOURCE_CAPACITY] = { 0 }; b3MeshData* meshes[SCENARIO_RESOURCE_CAPACITY] = { 0 }; b3HeightFieldData* heightFields[SCENARIO_RESOURCE_CAPACITY] = { 0 }; int jointCount = 0; int observations = 0; int eventObservations = 0; FILE* observationOutput = tmpfile(); FILE* hashOutput = tmpfile(); FILE* eventOutput = tmpfile();
    ...
    for (int i = 0; i < record->commandCount; ++i) {
        ...
        case SCENARIO_BODY: case SCENARIO_SPAWN: { if (!B3_IS_NON_NULL(world) || c->a < 0 || c->a >= SCENARIO_BODY_CAPACITY) return 2; b3BodyDef d = b3DefaultBodyDef(); d.type = (b3BodyType)c->kind; d.position = (b3Pos){ f32(c->values[0]), f32(c->values[1]), f32(c->values[2]) }; d.rotation = q4(c, 3); d.linearVelocity = v3(c, 7); d.angularVelocity = v3(c, 10); d.linearDamping = f32(c->values[13]); d.angularDamping = f32(c->values[14]); d.sleepThreshold = f32(c->values[15]); d.isBullet = HAS(c, 61); bodies[c->a] = b3CreateBody(world, &d); break; }
        ...
        case SCENARIO_HULL_RESOURCE: if (c->hull == NULL || c->a < 0 || c->a >= SCENARIO_RESOURCE_CAPACITY || (hulls[c->a] = make_hull(c->hull)) == NULL) return 2; break;
        case SCENARIO_MESH_RESOURCE: if (c->mesh == NULL || c->a < 0 || c->a >= SCENARIO_RESOURCE_CAPACITY || (meshes[c->a] = make_mesh(c->mesh)) == NULL) return 2; break;
        case SCENARIO_HEIGHT_RESOURCE: if (c->heightField == NULL || c->a < 0 || c->a >= SCENARIO_RESOURCE_CAPACITY || (heightFields[c->a] = make_height_field(c->heightField)) == NULL) return 2; break;
        ...
        case SCENARIO_FILTER: { if (!B3_IS_NON_NULL(world) || c->a < 0 || c->a >= SCENARIO_BODY_CAPACITY || c->b < 0 || c->b >= SCENARIO_BODY_CAPACITY || !b3Body_IsValid(bodies[c->a]) || !b3Body_IsValid(bodies[c->b])) return 2; b3FilterJointDef d = b3DefaultFilterJointDef(); d.base.bodyIdA = bodies[c->a]; d.base.bodyIdB = bodies[c->b]; if (!b3Joint_IsValid(b3CreateFilterJoint(world, &d))) return 2; jointCount++; break; }
