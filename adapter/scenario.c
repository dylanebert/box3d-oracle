...
static int run_scenario(int index, FILE* out)
{
    ...
    for (int i = 0; i < record->commandCount; ++i) {
        ...
        case SCENARIO_SHAPE: { if (!b3Body_IsValid(bodies[c->b])) return 2; b3ShapeDef d = b3DefaultShapeDef(); d.baseMaterial.rollingResistance = f32(c->values[1]); d.filter.groupIndex = (int)c->values[2]; d.density = f32(c->values[15]); d.updateBodyMass = !HAS(c, 57); d.invokeContactCreation = !HAS(c, 58); d.isSensor = (c->flags & SHAPE_SENSOR) != 0; d.enableSensorEvents = (c->flags & SHAPE_SENSOR_EVENTS) != 0; int resource = (int)c->values[0]; if (c->kind == 1) shapes[c->a] = b3CreateHullShape(bodies[c->b], &d, &boxes[resource].base); else if (c->kind == 2) shapes[c->a] = b3CreateSphereShape(bodies[c->b], &d, &spheres[resource]); else if (c->kind == 3) shapes[c->a] = b3CreateCapsuleShape(bodies[c->b], &d, &capsules[resource]); else if (c->kind == 4) shapes[c->a] = b3CreateMeshShape(bodies[c->b], &d, meshes[resource], v3(c, 3)); else if (c->kind == 5) shapes[c->a] = b3CreateHeightFieldShape(bodies[c->b], &d, heightFields[resource]); else if (c->kind == 7) shapes[c->a] = b3CreateHullShape(bodies[c->b], &d, hulls[resource]); else if (c->kind == 6 && c->compound != NULL) { b3CompoundData* compound = make_compound(c->compound, boxes, spheres, capsules, meshes); if (!compound) return 2; shapes[c->a] = b3CreateBakedCompoundShape(bodies[c->b], &d, compound); b3DestroyCompound(compound); } else return 2; if (!b3Shape_IsValid(shapes[c->a])) return 2; break; }
